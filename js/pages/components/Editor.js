...
export default {
    ...
    computed: {
        ...
        unfilledFieldNames() {
            ...
            return this.fields
                .filter((f) => {
                    const v = this.fieldValues ? this.fieldValues[f.name] : undefined;
                    if (v === undefined || v === null) return true;
                    const trimmed = String(v).trim();
                    if (trimmed === '') return true;
                    const def = f && f.default !== undefined && f.default !== null ? String(f.default).trim() : '';
                    return def !== '' && trimmed === def;
                })
                .map((f) => f.name);
        }
    },
        ...
        async onShareClick() {
            ...
            this.$emit('request-share', {
                ...
                continueShare: async () => {
                    ...
                    // Drop fieldValues that don't correspond to a current field — leftovers from
                    // previous templates inflate the URL and leak unrelated state.
                    if (state.fields && state.fieldValues) {
                        const keep = new Set(state.fields.map((f) => f.name));
                        const pruned = {};
                        for (const k of Object.keys(state.fieldValues)) {
                            if (keep.has(k)) pruned[k] = state.fieldValues[k];
                        }
                        state.fieldValues = pruned;
                    }
                    ...
                    if (utils.isDataTooLong(encoded, maxLength)) {
                        this.showShareToast(`Template is too large for a share link (${encoded.length}/${maxLength} chars). Try shortening default values, or copy the template manually.`, 'error');
                        return;
                    }
                        ...
                        <div class="flex items-center justify-between flex-wrap gap-y-2 mb-4">
                            <div class="flex items-center gap-3 flex-wrap">
                                ...
                                <div class="flex items-center bg-gray-900 rounded-full p-1 shadow-sm border border-gray-700">
                                    <button
                                        :class="['flex items-center px-3 sm:px-4 py-1 rounded-full transition font-semibold text-sm focus:outline-none',
